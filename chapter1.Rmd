...
description : This chapter will show you how to explore, refine, and visualize a sample of polling data from the 2016 Republican and Demcoratic primaries. You'll start with simple visualizations using base R commands before proceeding to more complicated plots in the ggplot2 package. The final exercises ask you to attach polling data to a map of the United States and to generate an interactive map using the googleVis package.
attachments :
...
Even the best polls are just an approximation of voter preferences. A poll taken three months, three weeks, or even three hours before the election still can't tell you exactly how someone will behave in the voting booth.
...
msg_bad <- "That's not correct! It may be true, but we can't tell from these polls."
...
Your previous plot of Democratic primary polls was difficult to interpret because it didn't show trends over time. To draw conclusions from polling data, analysts (and the general public!) often rely on crisp and clear visualizations. In this exercise, you'll use the `ggplot2` package to produce a more intuitive plot of Democratic primary polls.

In this exercise, you'll return to the complete dataset of all polls (`polls`), which is preloaded in your workspace.

*** =instructions
- Recreate your previous plot using the `ggplot2` package. Select only data from Democratic primaries (this time using `subset()`). Plot `percent` on the y-axis, `polldate` on the x-axis, and set color to `candidate`. Store your plot as `dem_plot`.
- View your new plot
...
*** =instructions
- Recreate your plot of the Democratic polling data using `polls2`. Save this plot as `dem_plot2`. Remember to use `subset` and `type == "dem"`.
...
Your plots look great! Now it's time to take a closer look at the polling data. Did you notice these polls are done at the state level? Especially during the primaries, campaigns tend to focus their effort on winning individual states, rather than maintaining popularity across the country. Instead of averaging across all the states, it might make sense to view polling data state-by-state.
...
*** =instructions
- Create a plot using `ggplot()` that includes only polling data for Republicans (`type == "rep"`) in the state of Iowa (`location == "IA"`) drawn from the `polls2` data. Save this plot to `ia_plot`.
...
Now that you've explored these polling data, you should have a better idea how to read, refine, and visualize polls across different units (in this case, different primary contests and states).
...
In this exercise, you'll use the `maps` package to generate a map of candidate support across each state. The `maps` package contains coordinates for a large number of important geographic and political units worldwide which R uses to generate maps. Before you can generate a map of polling data, however, you'll need to merge your polling data with geographic information about each state.

A new dataset containing only the final polling data for Bernie Sanders before each state's primary (`sanders`) has been preloaded into your workspace. the `ggplot2` package is also preloaded for you.


*** =instructions
- Load the `maps` package to gain access to geographic datsets and save the `states` data
- Merge `states` and `sanders` on the column `region`. Save this new data frame as `sanders_map`
- Reorder `sanders_map` according to `order` to make sure R knows how to plot your data
- Use ggplot to project Bernie Sanders' polling data onto a map of the United States

*** =hint
- Be sure to specify `states` and `sanders` as the two data frames for your `merge()` (in that order).
- Order the `sanders_map` data according to the `order` column (from the `states` data)
...
sanders_map <- merge(___, ___, by = ___, all=T)

# Reorder Sanders map according to the maps package order column
sanders_map <- ___[order(sanders_map$___ ), ]

# Use ggplot to produce a map of Bernie Sanders polling data (don't change this code)
ggplot() +
...
# Merge states and sanders by region: sanders_map
sanders_map <- merge(states, sanders, by = "region", all=T)
...
Your map looks excellent! Attaching polling data to a map of the United States allows for easy and intuitive identification of regional trends for each candidate. 

However, a color gradient alone makes it difficult to identify specific polling numbers from each state. Ideally, you want your map to display general trends as well as specific information about each state. To accomplish this, you'll use googleVis to generate an interactive map from your Bernie Sanders data.

The googleVis package allows you to create interactive charts and maps in R by providing a direct interface to the Google Charts API. Unlike the `maps` package, maps in `googleVis` do not require you to attach geographic coordinates to your data as long as they contain relevant geographic names (in this case, states). The `gvisGeoChart()` command requires you to specify data, a location variable, and a color variable.

Your Bernie Sanders polling data (`sanders`) is preloaded into your environment.

*** =instructions
- Load the `googleVis` package
- Generate a googleVis map object (`sanders_gvis`) by specifying `sanders` as the data, setting location to `location`, and setting color to `percent`
- Plot `sanders_gvis` using the base R plot command
...
test_function("gvisGeoChart", args = c("data", "locationvar", "colorvar")
              not_called_msg = "You didn't call `gvisGeoChart()`!",
              incorrect_msg = "You didn't call `gvisGeoChart(....)` with the correct arguments")

test_object("sanders_gvis")

test_error()

success_msg("Great job! Now that you have an interactive plot, you can get a general overview of trends across the United States as well as specific polling numbers for each state.")
```
